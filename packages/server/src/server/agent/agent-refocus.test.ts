...
// Durable finish reports (docs/finish-reports.md) are delivered as prompts through this same
// path, so a due refocus rides on one. The pair must still read as one system envelope: the
// timeline hides envelopes, and the owner's next refocus must not quote the report as direction.
test("a finish report can carry a refocus and stays a system envelope", async () => {
  const scenario = await createScenario({ enabled: true, growthTokens: 300_000 });
  await scenario.send("Coordinate the Android release.", { contextTokens: 20_000 });
  await scenario.send("Keep going.", { contextTokens: 380_000 });

  const report = formatSystemNotificationPrompt(
    formatFinishNotificationBody({
      childAgentId: "a1",
      title: "Fix the crash",
      reason: "finished",
  ...
  const carried = await scenario.send(report);

  expect(carried.startsWith(report)).toBe(true);
  expect(carried).toContain("<assignment>\nCoordinate the Android release.\n</assignment>");
  expect(isSystemInjectedEnvelope(carried)).toBe(true);
});
