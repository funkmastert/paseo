...
**Lowered** (`agents.processPriority.agentNice`): the agent provider processes (Claude, Codex, ACP agents and the commands they run through the daemon, Pi/OMP, OpenCode's shared server), and the terminals and workspace scripts an agent starts through the MCP tools. Root agents and child agents start at the same nice; nothing here tells them apart. Under saturation [the saturation rung](#the-saturation-rung) lowers the heaviest child agent trees further, and that is what puts roots ahead of children under pressure. **Lowered** (`backgroundNice`): the daemon's own periodic subprocesses, meaning the `git fetch` refresh and the forge PR-status polling (`gh`, `tea`, and the other forge CLIs). **Normal**: the daemon itself, terminals and scripts a person opens, and git or forge work someone is waiting on, such as opening a diff. A spawn site opts in with `priority` on `spawnProcess`/`execCommand`/`runGitCommand`, or `runWithSpawnPriority` around a call that reaches its subprocess through layers with no option to thread; nothing is lowered by default. The daemon never lowers itself: only root can raise a priority again on macOS and Linux.
