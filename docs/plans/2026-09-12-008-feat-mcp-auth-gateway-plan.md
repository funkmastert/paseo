...
- **Stop conditions:** Never restart the daemon on port 6767 or the Bozeo app; daemon-side code activates at the owner's next manual relaunch (staging built bundles is allowed). If evidence invalidates a session-settled KTD, stop and report rather than silently overriding.
...
- KTD1. **Reverse-proxy routes per brokered server, reusing the `/mcp/agents` pattern.** Sessions receive `{type:"http", url:"http://<listen>/mcp/gateway/<name>", headers:{Authorization:"Bearer <capability token>"}}` entries; the daemon relays MCP traffic to the upstream with its stored credentials. Mirrors `runtime-mcp-config.ts:45-57` (entry shape), `bootstrap.ts:1546-1558` (capability-token route auth), and `server/auth.ts:122` (`SELF_AUTHENTICATING_ROUTES` exemption — the gateway paths join it). Instantiates the session-settled "gateway proxy, not token sync" decision.
...
- KTD3. **OAuth: authorization-code + PKCE with a loopback callback route** (`/mcp/gateway/oauth/callback`), driven through the SDK's `OAuthClientProvider`. No inbound OAuth precedent exists in the daemon (verified) — this is the one genuinely new surface. Auth start is an RPC returning the authorization URL; clients open it via the existing external-URL opener (`desktop/src/main.ts:965-966` IPC; `@/utils/open-external-url` in the app).
...
Auth flow (F1/F3): strip button → `mcp_gateway.auth.start.request` → daemon builds PKCE authorization URL via `OAuthClientProvider` → client opens externally → provider redirects to loopback callback → daemon exchanges code, persists tokens (0600), flips state, emits `mcp_status_update`, upstream client (re)connects → proxy resumes; running sessions' next tool call succeeds (R4).

### Assumptions

- Upstream servers implement standard MCP OAuth (RFC 8414 discovery / dynamic client registration where offered) compatibly with the SDK's `OAuthClientProvider`; servers needing static tokens instead use `staticHeaders`.
...
- **Test scenarios:** state transitions incl. refresh-failure → needs-auth; token round-trip persists with 0600 and survives reload; malformed token file fails closed to needs-auth (never crashes); config live-patch round-trip; static-header servers connect without OAuth; disabled gateway constructs nothing.
...
- **Dependencies:** U1.
- **Files:** `packages/server/src/server/mcp-gateway/routes.ts`, `packages/server/src/server/bootstrap.ts` (route mount + `SELF_AUTHENTICATING_ROUTES` additions in `server/auth.ts`), tests (`mcp-gateway/routes.test.ts`, extend `agent-mcp.e2e.test.ts`-style local e2e with a fixture upstream MCP server).
- **Approach:** relay uses a per-server `StreamableHTTPServerTransport` facade forwarding JSON-RPC to the upstream client session (tool list/call passthrough); capability-token check identical to `bootstrap.ts:1546-1558` (reuse `isAgentMcpRequestAuthorized` with the same token); callback validates `state`, exchanges code via U1's oauth module, redirects to a tiny "you can close this tab" page.
...
- **Test scenarios:** unauthorized request → 401; authorized round-trip lists and calls a fixture tool; needs-auth upstream → relay returns a clean MCP error (never hangs); callback with bad state → 400, no token write; happy callback persists tokens and flips state.
...
- **Requirements:** R1, R3, R10, AE4. Cites KTD5, KTD6.
- **Dependencies:** U1, U2.
...
- **Test scenarios:** start on unknown server → error response; start on static-header server → error (nothing to auth); happy path returns a URL embedding PKCE challenge; permission mapping (`daemon.write`-class) enforced.
...
- **Approach:** collapsed row = aggregate dot + names of unhealthy critical servers; expand on press; rows use ProviderUsageTone + icon+label (never color alone); auth button calls the U6 RPC then opens the returned URL (`open-external-url`); show-nothing when the feature flag is absent (old daemon).
- **Execution note:** coordinate `left-sidebar.tsx` edits with the concurrent disk-meta-row agent; land second if it is mid-flight.
- **Test scenarios:** model — tones per state, critical-first ordering, collapsed summary counts, absent-data renders nothing; hook gated off when feature flag missing; i18n parity suite.
...
- No daemon restart, no plugin reload, no Bozeo relaunch; live verification of the full auth loop is deferred to the owner's next relaunch and explicitly out of the automated gates.
