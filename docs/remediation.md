...
| `cpu-saturation`            | [resource monitor](resource-monitor.md#the-saturation-rung)                                      | the reaper's pass, lowering the heaviest child agent trees, holding child admission | Only when the cause is unknown: find what loads the CPU, stop only provable leftovers | `notice` with a cause, `alert` without  |
