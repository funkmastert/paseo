...
Do not compare with `normalizeClaudeRuntimeModelId`. It maps ids onto the manifest for display, and any id the manifest lacks becomes null, so two different unknown models compare equal. Until its fallback regex was anchored it also turned `claude-opus-5-5` into `claude-opus-5`, which is how an Opus 5.5 agent's runtime info once read `claude-opus-5` while the CLI was serving Opus 5.5 (real stream: init, `message_start`, the assistant frame and `modelUsage` all report `claude-opus-5-5`). A comparison built on a manifest mapping cannot see the substitution it exists to catch. `canonicalModelId` only lowercases and strips the session suffix and the date.
