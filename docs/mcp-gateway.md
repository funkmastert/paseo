...
Claude sessions launched while the gateway is enabled receive the brokered servers as per-launch `mcpServers` entries (never persisted, stripped from storage like the `paseo` entry) and `strictMcpConfig` so per-dir remote definitions stop loading. Per-dir stdio entries from the session config dir's `.claude.json` and the project `.mcp.json` are re-read and re-injected with `${VAR}`/`${VAR:-default}` expansion, so local stdio servers behave as before. Local-scope entries (`projects.<dir>.mcpServers` inside `.claude.json`) are not re-injected in v1.

Per-session MCP statuses reported by the SDK at init are captured onto the agent (live-only) and surface in the strip as session-reported rows without an auth action.
