...
- `scope`: the space-separated scopes to request. Without it the SDK requests everything the resource advertises in `scopes_supported`, which for Slack includes posting, writing canvases, and uploading files as you. The daemon sends it as written and does not add `offline_access` to it, so list `offline_access` yourself if the provider needs it for a refresh token.
...
Claude sessions launched while the gateway is enabled receive the brokered servers as per-launch `mcpServers` entries (never persisted, stripped from storage like the `paseo` entry). That includes every derived provider (`extends: "claude"`), which is every account-pool account: the manager asks the client (`acceptsMcpGatewayServers`), not the provider id, and `wrapClientProvider` forwards it. Gating on `provider === "claude"` once left `claude-personal` and `claude-backup` loading their own per-dir `zeeq`, `linear` and `notion`, each with its own login, while the gateway held working tokens for all three.

An agent whose labels carry `paseo.mcp-scope` gets only the brokered servers that label lists (`none` for no server). Unless the list includes `claude.ai`, the launch also sets the CLI's `disableClaudeAiConnectors` setting, which drops the account's claude.ai connectors; the CLI loads those from the account, so no injected config can remove them. That half applies to Claude clients whether or not the gateway is on. The manager reads the label on every launch (create, resume, reload), so an agent keeps the set it was spawned with. Adding or dropping a server mid-session rebuilds the prompt cache. An agent without the label gets every server, including every agent created before scoping existed. The account-pool plugin's classifier writes the label on spawned children and leaves roots unscoped; the plugin README's [MCP servers](../plugins/claude-account-pool/README.md#mcp-servers) section covers what it grants and the `paseo.mcp` label that asks for more. Paseo's own `paseo` entry is never scoped.

`sessionMode` decides what happens to everything else the CLI would load:

- `overlay` (default): the brokered entries ride `--mcp-config` next to the CLI's own user, project, and local scopes and its claude.ai connectors. A launch-time entry wins a name collision with a per-dir entry, so a brokered `github` shadows a stale user-scope `github`. Per-dir entries the gateway does not broker keep loading and keep failing on their own; remove them with `claude mcp remove <name> -s user` in that account's config dir. That includes a server an agent's scope leaves out: its per-dir entry loads in the brokered entry's place. With its own login gone stale it reports needs-auth and offers no tools, but an account whose per-dir login still works puts that server's tools back.
